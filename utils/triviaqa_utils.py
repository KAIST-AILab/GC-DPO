...
def get_qd_to_answer(data):
    ...
    for datum in data['Data']:
        ...
        key_to_answer[dtum['QuestionId']] = datum["Answer"]
...
def get_context(args, example):
    entire_content = ""
    num_para = 0
    if len(example["EntityPages"]) != 0:
        for ep in example["EntityPages"]:
            try:
                file_path = args.context_dir + "/wikipedia/" + ep['Filename']
                
                entire_content += get_text_file(file_path) + "/n"
            except FileNotFoundError:
                file_path = args.context_dir + "/web/" + ep['Filename']
                entire_content += get_text_file(file_path) + "/n"
            num_para += 1

    elif len(example["SearchResults"]) != 0:
        for ep in example["SearchResults"]:
            try:
                file_path = args.context_dir + "/web/" + ep['Filename']
